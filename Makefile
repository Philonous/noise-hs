...
.PHONY: build
build: dist/wireguard $(tests)

$(tests): dist/tests/% : dist/wireguard $(test-srcfiles) package.yaml stack.yaml
	mkdir -p dist/tests
	cp "$(shell stack ${stack_args} path --dist-dir)/build/$(notdir $@)/$(notdir $@)" dist/tests/

dist/wireguard: $(srcfiles) $(test-srcfiles) package.yaml stack.yaml
	rm -f *.cabal
